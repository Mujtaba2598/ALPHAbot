...
class AutoCompoundingEngine {
    async executeTrade(sessionId, userEmail, apiKey, secretKey, config) {
        ...
        // Auto‑compounding: position size grows with current profit
        // We treat currentProfit as virtual balance increase; we reinvest it.
        const currentVirtualBalance = initialInvestment + currentProfit;
        const remainingTarget = targetProfit - currentProfit;
        
        // Base size as a percentage of current virtual balance (more aggressive as we progress)
        let basePercent = 0.3; // 30% of virtual balance per trade
        if (remainingTarget < targetProfit * 0.3) basePercent = 0.5; // last 30% of target -> 50% per trade
        if (remainingTarget < targetProfit * 0.1) basePercent = 0.8; // final sprint -> 80% per trade
        
        let positionSize = currentVirtualBalance * basePercent * timePressure;
        // Ensure we don't exceed actual available balance
        const actualBalance = await getRealBalance(apiKey, secretKey);
        positionSize = Math.min(positionSize, actualBalance * 0.9); // never use more than 90% of real balance
        ...
        if (actualBalance < positionSize) {
            console.log(`⚠️ Insufficient balance: need ${positionSize} USDT, have ${actualBalance} USDT. Skipping trade.`);
        ...
        try {
            ...
            activeTradingSessions[sessionId].currentProfit += profit; // add real profit
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Trade size grows automatically with profits`);
